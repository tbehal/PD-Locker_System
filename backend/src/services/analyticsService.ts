...
import type { RegistrationRow, RegistrationResult } from '../types/hubspot';
...
async function resolveCycles(year: number, cycleId: number | null): Promise<Cycle[]> {
  if (cycleId) {
...
async function getSeatingAnalytics(
  year: number,
  cycleId: number | null = null,
): Promise<SeatingAnalyticsResult> {
  ...
  const bookingMatrix: Record<string, Record<number, number>> = {};
  const labStationCounts: Record<string, number> = {};
  ...
  const shiftOccupancy = (['AM', 'PM'] as const).map((shift) => {
...
async function getRegistrationAnalytics(
  ...
  shift: string,
  cycleId: number | null = null,
): Promise<RegistrationAnalyticsResult> {
  const cycles = await resolveCycles(year, cycleId);
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    if (result.status === 'fulfilled') {
      allRows.push(...(result.value as RegistrationResult).rows);
    } else {
      const err = result.reason as AppErrorLike;
      const msg =
...
export { getSeatingAnalytics, getRegistrationAnalytics };
